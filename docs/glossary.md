...
## UI 명칭 (화면에 보이는 말 — 코드 식별자와 구분)

- **책장** — 등록한 책 전체를 보는 화면(`books`). 홈의 `책장 →` 링크가 가리키는 곳. 책 모음을 부르는 **유일한** 이름이다(홈 h1 `내 책`만 예외 — 화면 성격을 가리킴).
- **독서 시작** — `startNewSession()`을 부르는 **그 버튼 하나만**. 그 책의 열린 기록을 닫고 목적(project)을 붙여 새로 시작한다. 화면 이동 버튼에 붙이지 말 것(2026-08-11 정정 — 홈의 동명 버튼은 실제로는 책장 이동이라 `새 책 시작하기`로 바꿨다).
- **새 책 시작하기** — 홈 CTA. 책장(새 책 폼이 맨 위)으로 이동한다.
- **새 책 추가** — 책장의 폼 CTA. 누르는 즉시 책이 저장되고 목적 입력 단계로 넘어간다.
- **검색** — 홈 상단 입력에서 시작하는 **전역 검색**(ADR-022). 책 제목·저자와 캡처 본문(담은 글·내 생각·레거시 why)을 한 번에 찾고 결과를 **책별로 묶어** 보여준다. 캡처 조회는 이미지를 싣지 않는 `searchCaptures()`만 쓴다. `ocr`은 죽은 필드라 대상이 아니다.
- **사진 / 입력** — 책 카드의 두 버튼. `currentRoundFor()`로 **열린 기록에 이어서** 캡처한다(새로 시작하지 않는다). "독서 시작"과 의미가 다르다는 점에 주의.
