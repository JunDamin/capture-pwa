...
- **Image (사진)** — 선택, **캡처당 최대 2장**. 책 페이지 등의 사진. 리사이즈+압축된 Blob로 저장, 원본 미보관. 저장은 슬롯 2개(`image`/`image2`)지만 읽기는 전부 `capturePhotos(c): Photo[]` 접근자를 쓴다. **2장의 의미는 고정 — 같은 글의 앞 페이지 / 뒤 페이지**(임의의 사진 모음이 아님). Export에서 `capture-NNa` / `capture-NNb`로 갈리며 AI는 둘을 이어 하나의 원문으로 전사한다. (ADR-001, ADR-003, ADR-020)
...
- **사진 경로 (빠른 경로)** — 카메라 항상 켜짐 → 찰칵 → **편집 시트 상승**(사진 확인·다시 찍기·사진 추가·태그·담은 글·생각·페이지) → 저장 → 시트 하강, 라이브 복귀. 사진이 있으면 **태그만 필수**. **3초 예산 적용**, 타이핑 없이도 완료 가능(태그+저장 2탭). (ADR-001, ADR-018)
- **사진 추가** — 편집 시트에서 2장째를 붙이는 동선(글이 다음 페이지로 이어질 때). 탭하면 사진·텍스트·태그를 보존한 채 시트만 하강하고, 다음 셔터가 2장째를 붙이며 시트를 재개한다. 1장만 찍는 경로의 탭 수는 불변. (ADR-020)
